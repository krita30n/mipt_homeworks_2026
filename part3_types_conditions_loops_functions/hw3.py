...
def cost_handler(category_name: str, amount: float, income_date: str) -> str:
    ...
    if date is None:
        return INCORRECT_DATE_MSG
    financial_transactions_storage[-1] = {"category": category_name, "amount": amount, "date": date}
    return OP_SUCCESS_MSG
...
def denormalize_num(num: float) -> str:
...
def format_stats_output(
    ...
    total_capital_str = denormalize_num(total_capital)
    unsigned_total_capital_str = denormalize_num(unsigned_total_capital)
    costs_amount_str = denormalize_num(costs_amount)
    incomes_amount_str = denormalize_num(incomes_amount)

    category_details_list: list[str] = []
    for idx, (category, amount) in enumerate(category_details.items(), start=1):
        amount_str = denormalize_num(amount)
...
def main() -> None:
    while True:
        ...
        user_input_parts = normalize_num(user_input_str).split()
